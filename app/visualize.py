...
import json
with open('joint_positions.json', 'r') as fp:
        data=json.load(fp)
images = []

for joint_positions in list(data.values())[:5]:
    joint_positions=list(joint_positions.values())[0]
    fig = plt.figure()
    ax = plt.axes(projection='3d')
    ...
    set_axes_equal(ax)    
    fig.canvas.draw()
    images.append(PIL.Image.frombytes('RGB', fig.canvas.get_width_height(),fig.canvas.tostring_rgb()))

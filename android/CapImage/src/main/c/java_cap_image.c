...
JNIEXPORT void JNICALL Java_com_example_capimage_CapImage_init(JNIEnv *env, jclass clazz) {
	cap_init();
...
JNIEXPORT void JNICALL Java_com_example_capimage_CapImage_pause(JNIEnv *env, jclass clazz) {
	cap_pause();
}

/* static native void resume() */
JNIEXPORT void JNICALL Java_com_example_capimage_CapImage_resume(JNIEnv *env, jclass clazz) {
	cap_resume();
}

/* static native short[] readImage(); */
JNIEXPORT jshortArray JNICALL Java_com_example_capimage_CapImage_readImage(JNIEnv *env, jclass clazz) {
    if(cap_request() < 0) {
...
JNIEXPORT jshortArray JNICALL Java_com_example_capimage_CapImage_waitForTouch(JNIEnv *env, jclass clazz, jobject jtouch) {
    int size = cap_image_width * cap_image_height;
    jshortArray result = (*env)->NewShortArray(env, size);
    if(result == NULL)
        return NULL;
...
JNIEXPORT jboolean JNICALL Java_com_example_capimage_CapImage_setMode(JNIEnv *env, jclass type, jint mode) {
